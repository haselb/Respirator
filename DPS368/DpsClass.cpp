...
//  delay(calcBusyTime(0U, m_prsOsr) / DPS__BUSYTIME_SCALING);
	delay100us((calcBusyTime(0U, m_prsOsr) / DPS__BUSYTIME_SCALING)*10U);
...
/*	m_i2cbus->beginTransmission(m_slaveAddress);
	...
	Soft_WDG_1(ON);
	I2C_MASTER_Transmit(&I2C_MASTER_0,false,m_slaveAddress,&data,1,true);
	while((tx_completion_0 == 0) && (flag_I2C_NACK == 0) && (flag_eject_at_I2C_NACK == 0));
	tx_completion_0 = 0;
	Soft_WDG_1(OFF);  // if after xxms of no response, IRQ set flag_eject_at_I2C_NACK=1 to leave while loop
	if(flag_I2C_NACK || flag_eject_at_I2C_NACK)
...
*/
	...
	Soft_WDG_1(ON);
	I2C_MASTER_Transmit(&I2C_MASTER_0,true,m_slaveAddress,&regBlock.regAddress,1,false);
	while((tx_completion_0 == 0) && (flag_I2C_NACK == 0) && (flag_eject_at_I2C_NACK == 0));
	tx_completion_0 = 0;
	Soft_WDG_1(OFF);
